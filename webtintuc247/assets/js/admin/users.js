...
function showNotification(text, type = "success") {
  ...
  if (!msgDiv) {
    ...
    msgDiv.style.cssText =
      "padding: 15px; margin-bottom: 20px; border-radius: 4px; color: white; font-weight: bold; transition: opacity 0.5s; z-index: 9999;";
  ...
  if (tbody) {
    ...
    if (checkAll) {
      checkAll.addEventListener("change", function () {
        document
          .querySelectorAll(".user-check:not([disabled])")
    ...
    tbody.addEventListener("click", (e) => {
      if (e.target.classList.contains("btn-delete-item")) {
        ...
        if (id && status) toggleUserStatus(id, status);
      }
    });
  }
...
async function loadUsers() {
  ...
  const keyword = keywordInput
    ? encodeURIComponent(keywordInput.value.trim())
    : "";
  ...
  try {
    ...
    tbody.innerHTML =
      '<tr><td colspan="6" style="color:red; text-align:center;">Lỗi kết nối máy chủ!</td></tr>';
  }
}
...
function renderTable(data, currentAdminId) {
  ...
  data.forEach((row) => {
    ...
    if (row.role === "admin")
      roleHtml =
        '<span class="status-badge" style="background:#fce4ec; color:#c2185b;">Admin</span>';
    else if (row.role === "phongvien")
      roleHtml =
        '<span class="status-badge" style="background:#fff8e1; color:#f57f17;">Phóng Viên</span>';
    else if (row.role === "editor")
      roleHtml =
        '<span class="status-badge" style="background:#e8f5e9; color:#2e7d32;">Biên Tập Viên</span>';
    else if (row.role === "nhabao")
      roleHtml =
        '<span class="status-badge" style="background:#f3e5f5; color:#7b1fa2;">Nhà Báo</span>';
    else if (row.role === "ctv")
      roleHtml =
        '<span class="status-badge" style="background:#e0f2f1; color:#00796b;">Cộng Tác Viên</span>';
    else
      roleHtml =
        '<span class="status-badge" style="background:#e3f2fd; color:#1565c0;">Người dùng</span>';

    let statusHtml = row.status === 'locked' 
        ? '<span class="status-badge" style="background:#ce5b5b; color:#fff;">Bị khóa 🔒</span>'
        : '<span class="status-badge" style="background:#e8f5e9; color:#2e7d32;">Hoạt động</span>';

    let isSelf = row.id == currentAdminId;

    let actions = `<a class="btn btn-edit" href="${BASE_URL}admin/users/edit/${row.id}">✏️ Sửa</a>`;

    // Không cho phép xóa/khóa chính mình
    if (isSelf) {
      actions += `<button type="button" class="btn btn-delete btn-disabled" disabled style="opacity:0.5" title="Bạn không thể thao tác chính mình">🗑️ Xoá</button>`;
    } else {
      let lockBtnText = row.status === 'locked' ? 'Mở khóa' : 'Khóa';
      actions += `<button type="button" class="btn btn-view btn-toggle-status" data-id="${row.id}" data-status="${row.status === 'locked' ? 'active' : 'locked'}">🔒 ${lockBtnText}</button>`;
      actions += `<button type="button" class="btn btn-delete btn-delete-item" data-id="${row.id}">🗑️ Xoá</button>`;
    }

    let avatar = row.avatar
      ? `<img src="${BASE_URL}assets/images/avatars/${row.avatar}" style="width:30px; height:30px; border-radius:50%; vertical-align:middle; margin-right:10px; object-fit:cover;">`
      : `<div style="display:inline-block; width:30px; height:30px; border-radius:50%; background:#ccc; vertical-align:middle; margin-right:10px;"></div>`;
...
function updateDeleteBtn() {
  ...
  document.getElementById("checkAll").checked =
    document.querySelectorAll(".user-check:not([disabled])").length > 0 &&
    c === document.querySelectorAll(".user-check:not([disabled])").length;
...
async function deleteMultiple() {
  const cbs = Array.from(document.querySelectorAll(".user-check:checked")).map(
    (cb) => cb.value,
  );
  if (
    cbs.length == 0 ||
    !confirm(`Bạn có chắc chắn muốn xoá ${cbs.length} người dùng đã chọn?`)
  )
    return;
...
async function initUserForm() {
  try {
    ...
    if (rItem.status === "success") {
      ...
      document.getElementById("status").value = rItem.data.status || "active";
  ...
  try {
    ...
    if (r.status === "success") {
      window.location.href =
        BASE_URL + "admin/users?msg=" + (isEdit ? "updated" : "added");
    } else {
...
async function toggleUserStatus(id, newStatus) {
  try {
    const res = await fetch(BASE_URL + "api/users/" + id, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ quick_toggle: true, status: newStatus }),
    });
    const result = await res.json();
    if (result.status === "success") {
      showNotification(result.data || result.message, "success");
      loadUsers();
    } else {
      showNotification(result.message, "error");
    }
