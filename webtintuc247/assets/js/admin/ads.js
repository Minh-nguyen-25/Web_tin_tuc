document.addEventListener('DOMContentLoaded', () => {
    ...
    if (tbody) {
        ...
        tbody.addEventListener('click', (e) => {
            ...
            const toggleBtn = e.target.closest('.btn-toggle-ads');
            if (toggleBtn) {
                const id = toggleBtn.getAttribute('data-id');
                if (id) toggleAdsStatus(id);
            }
        });
    }
                ...
                <tr>
                    ...
                    <td>
                        <div class="action-buttons">
                            ...
                            <button type="button" class="btn btn-view btn-toggle-ads" data-id="${row.id}" title="Bật/Tắt hiển thị">
                                ${row.status === 'hien' ? '👁️ Ẩn' : '⏵️ Hiện'}
                            </button>
...
async function toggleAdsStatus(id) {
    try {
        const res = await fetch(BASE_URL + 'api/ads/' + id, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ quick_toggle: true })
        });
        const result = await res.json();
        if (result.status === 'success') {
            showMsg(result.data || result.message);
            loadAds();
        } else {
