    ...
    if (tbody) {
        ...
        if (filterSelect) {
            filterSelect.addEventListener('change', () => {
                // Khi chọn danh mục thì xoá từ khoá để tránh xung đột
                const fk = document.getElementById('keyword');
                if (fk) fk.value = '';
                loadCategories();
            });
        }
...
async function loadCategories() {
    ...
    try {
        ...
        if (result.status === 'success') {
            ...
            // Luôn populate lại select để giữ đúng danh sách
            ...
            const filterSelectEl = document.getElementById('filterId');
            if (filterSelectEl) {
                const currentVal = filterSelectEl.value;
                filterSelectEl.innerHTML = filterHtml;
                filterSelectEl.value = currentVal; // giữ nguyên lựa chọn hiện tại
            }
