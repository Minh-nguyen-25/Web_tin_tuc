...
class Users extends ApiController
{
    ...
    public function store()
    {
        ...
        $userModel->create($username, $password, $hoten, $email, $role, $status);
    ...
    public function update($id = 0)
    {
        ...
        if (isset($input['quick_toggle']) && $input['quick_toggle'] === true) {
            $userModel = $this->model('UserModel');
            $user = $userModel->getById((int)$id);
            if (!$user) $this->json('error', 'Người dùng không tồn tại');
            if ($id === (int)($_SESSION['admin_id'] ?? 0)) $this->json('error', 'Bạn không thể khóa chính mình');
            
            $newStatus = trim($input['status'] ?? 'active');
            $userModel->update((int)$id, $user->hoten, $user->email, $user->role, null, $newStatus);
            $this->json('success', 'Đã cập nhật trạng thái tài khoản');
        }
        ...
        $userModel->update((int)$id, $hoten, $email, $role, $passToUpdate, $status);
