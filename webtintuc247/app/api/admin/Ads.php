...
class Ads extends ApiController
{
    ...
    public function update($id = 0)
    {
        ...
        // Quick toggle status (không cần form đầy đủ)
        $jsonInput = json_decode(file_get_contents('php://input'), true);
        if (isset($jsonInput['quick_toggle']) && $jsonInput['quick_toggle'] === true) {
            $adsModel = $this->model('AdsModel');
            $ad = $adsModel->getById($id);
            if (!$ad) $this->json('error', 'Không tìm thấy quảng cáo');
            $newStatus = $ad->status === 'hien' ? 'an' : 'hien';
            $adsModel->updateStatus($id, $newStatus);
            $msg = $newStatus === 'hien' ? 'Đã bật hiển thị quảng cáo' : 'Đã ẩn quảng cáo';
            $this->json('success', $msg);
        }
